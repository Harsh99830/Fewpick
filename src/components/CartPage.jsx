...
export default function CartPage({ cartItems, onUpdateQty, onNavigateHome, orderingEnabled = true, closedMessage }) {
  ...
  // State for Rider's Effort tip selection: null (no selection by default), 10, 15, 20, or custom
  const [riderTip, setRiderTip] = useState(null);
  ...
  // Is Rider's Effort option selected & valid? (must click 10, 15, 20 or enter custom tip between ₹10 and ₹50)
  const isTipSelected = (isCustomTip && isValidCustomTip) || (!isCustomTip && riderTip !== null);

  // Computations
  const subtotal = cartItems.reduce((acc, item) => acc + (item.product.price * item.quantity), 0);
  
  // Delivery Charge / Rider's Effort calculation (0 if unselected or invalid)
  const deliveryCharge = subtotal > 0
    ? (isCustomTip ? (isValidCustomTip ? parsedCustomTip : 0) : (riderTip !== null ? riderTip : 0))
    : 0;
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 p-4 sm:p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2.5 text-xs sm:text-sm">
            ...
            <div className="flex flex-col gap-2 border-b border-dashed border-gray-200 pb-3">
              {/* Single row with Rider's Effort label + Price Pill Options */}
              <div className="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap">
                <div className="flex flex-col flex-shrink-0">
                  <span className="text-xs sm:text-sm font-semibold text-gray-700">Rider's Effort</span>
                </div>

                {/* Inline Price Option Pills */}
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setRiderTip(riderTip === amount ? null : amount);
                        }}
          ...
          {!orderingEnabled ? (
            ...
            <div className="flex flex-col gap-1.5">
              <button
                ...
                disabled={isCheckingOut || !isTipSelected}
                className={`w-full py-3.5 text-white font-extrabold rounded-xl transition-all text-sm sm:text-base flex items-center justify-center gap-2 border-none ${
                  !isTipSelected
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none'
                    : 'bg-[#00A859] hover:bg-[#00964f] shadow-[0_4px_16px_rgba(0,168,89,0.3)] cursor-pointer active:scale-[0.99]'
                }`}
              >
                {isCheckingOut ? (
              ...
              {!isTipSelected && (
                <p className="text-[10.5px] font-bold text-amber-600 text-center m-0">
                  Select Rider's Effort to place order
                </p>
              )}
            </div>
          )}
